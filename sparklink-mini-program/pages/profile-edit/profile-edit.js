...
Page({
  data: {
    // 勿用空串：否则 network-image 从「无 url」到「有 url」时部分机型/基础库不刷新，出现白块
    avatar: '/images/default-avatar.png',
    /** 变更后自增，强制 custom component 重挂载，避免同页二次进入仍白屏 */
    _avatarViewKey: 0,
    nickname: '',
    gender: '',
    birthday: '',
  ...
  onShow() {
    // 每次进入编辑页都拉最新资料（与「我的」只 onShow 拉取一致；避免页面实例被复用时不跑 onLoad 导致仍显示旧/空头像）
  ...
  fillFormFromUserInfo(userInfo) {
    ...
    avatar = resolveMediaUrl(avatar, { baseUrl, kind: 'avatar' });
    ...
    this.setData({
      ...
      _userInfo: userInfo,
      _avatarViewKey: (this.data._avatarViewKey || 0) + 1
    });
  },
  ...
  uploadAvatar(filePath) {
    ...
    wx.getFileInfo({
      filePath,
      success: (info) => {
        if (info && info.size > maxB) {
          wx.showModal({
            title: '图片过大',
            content: '请选择 5MB 以内的图片，或先用系统工具压缩后再选。',
            showCancel: false
          });
          return;
        }
        this._doUploadAvatar(filePath, baseUrl, token, userId);
      },
      fail: () => {
        this._doUploadAvatar(filePath, baseUrl, token, userId);
      }
  ...
  _doUploadAvatar(filePath, baseUrl, token, userId) {
  ...
  onSave() {
    ...
    if (isTempLocalUrl(avatarVal)) {
      ...
      avatarVal = resolveMediaUrl(avatarVal, { baseUrl: base, kind: 'avatar' });
    }
    const payload = {
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        if (res.statusCode === 200 && data && (data.code === 0 || data.code === 200)) {
          ...
          // 头像以服务端落库/规范化结果为准，避免长 URL 与 /static/ 混存导致下次 GET 后展示异常
          const nextAvatar = (updated.avatar != null && String(updated.avatar).trim() !== '')
            ? String(updated.avatar).trim()
          ...
          wx.setStorageSync('userInfo', merged);
          // 与登录态一致：聊天页等仍读取 userAvatar，避免仅改 userInfo 时会话里头像不更新
          let syncAv = nextAvatar || '/images/default-avatar.png';
          if (syncAv && (syncAv.indexOf('__tmp__') !== -1 || syncAv.indexOf('://tmp/') !== -1)) {
            syncAv = existing.avatar || wx.getStorageSync('userAvatar') || '/images/default-avatar.png';
          } else if (syncAv && !syncAv.startsWith('http') && syncAv !== '/images/default-avatar.png') {
            syncAv = resolveMediaUrl(syncAv, { baseUrl: base, kind: 'avatar' });
          }
          if (app.normalizeImageUrl && syncAv) {
            syncAv = app.normalizeImageUrl(syncAv, base) || syncAv;
          }
          wx.setStorageSync('userAvatar', syncAv);
