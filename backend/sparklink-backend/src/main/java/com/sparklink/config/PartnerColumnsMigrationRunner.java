...
public class PartnerColumnsMigrationRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        try (Connection conn = dataSource.getConnection()) {
            ...
            if (!hasColumn(conn, TABLE, "plan_end_time")) {
                try (Statement stmt = conn.createStatement()) {
                    stmt.executeUpdate(
                            "ALTER TABLE t_partner ADD COLUMN plan_end_time DATETIME DEFAULT NULL COMMENT '计划/活动结束时间' AFTER plan_time");
                    log.info("[迁移] t_partner.plan_end_time 已添加");
                }
            }
            // 不再将 scope=3 批量改为 4：位掩码下 3=1|2=「公开+同事」为合法值，若执行 UPDATE 会误伤多选为公开+公司可见的数据。
        } catch (Exception e) {
