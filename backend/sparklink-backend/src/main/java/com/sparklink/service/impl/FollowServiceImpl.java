...
import com.sparklink.mapper.FollowMapper;
...
public class FollowServiceImpl implements FollowService {
    ...
    public void followUser(Long userId, Long followUserId) {
        ...
        // 取关为逻辑删除时，库中仍占唯一键；再次关注应恢复该行，禁止再 insert 一条
        int restored = followMapper.restoreIfLogicallyDeleted(userId, followUserId);
        if (restored > 0) {
            return;
        }
