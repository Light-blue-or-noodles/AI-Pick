...
SET @exist := (SELECT COUNT(*) FROM information_schema.columns
               WHERE table_schema = 'sparklink' AND table_name = 't_follow' AND column_name = 'deleted');
SET @sql := IF(@exist = 0,
...
SET @exist2 := (SELECT COUNT(*) FROM information_schema.columns
                WHERE table_schema = 'sparklink' AND table_name = 't_favorite' AND column_name = 'deleted');
